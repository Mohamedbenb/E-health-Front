...
@Component({
  ...
  template:`<a [routerLink]="['../../pages/details']" [state]="{data:rowData, source:'tableau'}" (click)="setData($event)" class="link-style">{{ value }}</a>
  `,

})
export class LinkComponent2 implements OnInit, ViewCell {
  @Input() value: any;
  @Input() rowData: any;
  @Input() Status: any;
  ...
  setData(event){
    ...
    localStorage.setItem('source','tableau')

...
@Component({
  ...
  template:`<a [routerLink]="['../../pages/details']" [state]="{data:rowData, source:'exam'}" (click)="setData($event)" class="link-style">{{ value }}</a>
  `,

})
export class LinkComponent3 implements OnInit, ViewCell {
  @Input() value: any;
  @Input() rowData: any;
  @Input() Status: any;
  ...
  setData(event){
    ...
    localStorage.setItem('source','exam')
